...
def next_run_index(payload_g):
	for i in range(1, 100):
    	fn = "p{:02d}_rls3fix_{:02d}.csv".format(payload_g, i)
    	try:
        	open(fn, "r").close()
    	except OSError:
        	return i
...
def run_sysid(payload_g):
	...
	filename = "p{:02d}_rls3fix_{:02d}.csv".format(payload_g, run_id)

	v_lpf = LowPass(alpha_from_tau(TAU_V, DT_TARGET), 0.0)
	a_lpf = LowPass(alpha_from_tau(TAU_A, DT_TARGET), 0.0)

	est = RlsEstimator(lam=0.998, p0=80.0)

	f = open(filename, "w")
	f.write("t,U,cmd,dist_m,v_mps,v_filt_mps,a_enc_mps2,a_imu_mps2,s_hat,bc_hat,k_hat,upd\n")

	display.fill(0)
	display.text("SYSID RLS FIX", 0, 0)
	display.text("still for bias", 0, 10)
	display.show()
	...
	while True:
    	...
    	a_enc = (vf - last_vf) / dt
    	last_vf = vf
    	a_enc_f = a_lpf.update(a_enc)

    	a_imu = 0.0
    	if USE_IMU:
        	a_imu = read_forward_accel_mps2(imu) - a_bias

    	upd = 0
    	if (t > WARMUP_S) and (abs(vf) > MIN_SPEED) and (abs(a_enc_f) < MAX_ABS_ACCEL):
        	if U < 0.02:
            	est.update(U=0.0, v=vf, y=a_enc_f)
        	else:
            	est.update(U=U, v=vf, y=a_enc_f)    
        	upd = 1

    	s_hat, bc_hat, k_hat = est.theta
    	f.write("{:.3f},{:.3f},{:d},{:.4f},{:.6f},{:.6f},{:.6f},{:.6f},{:.6f},{:.6f},{:.6f},{}\n".format(
        	t, U, cmd, dist_m, v, vf, a_enc_f, a_imu, s_hat, bc_hat, k_hat, upd
    	))
	...
	print("Distance:", dist_m, "m")
	print("Final [s, bc, k] =", est.theta)

print("=== RUN SYSID RLS FIX ===")
